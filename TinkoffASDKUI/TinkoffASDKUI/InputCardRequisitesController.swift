...
class InputCardRequisitesController: NSObject {
    ...
    private func activateScanerButton() {
        if cardRequisitesScaner != nil {
            ...
            inputView?.buttonRight.setImage(UIImage(named: "scan", in: Bundle.tinkoffUI, compatibleWith: nil), for: .normal)
    ...
    private func activateNextButton() {
        inputView?.buttonRight.isHidden = false
        inputView?.buttonRight.setImage(UIImage(named: "next", in: Bundle.tinkoffUI, compatibleWith: nil), for: .normal)
